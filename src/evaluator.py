        ...
        with torch.no_grad():
            for data_type in ['valid', 'test']:
                ...
                if self.params.beam_eval:
                    self.enc_dec_step_beam(data_type, scores)
                else:
                    self.enc_dec_step(data_type, scores)

        return scores

    def enc_dec_step(self, data_type, scores):
        """
        ...
        if params.eval_verbose:
            eval_path = os.path.join(params.dump_path, f"eval.{scores['epoch']}")
            f_export = open(eval_path, 'w')
            logger.info(f"Writing evaluation results in {eval_path} ...")

        # iterator
        iterator = self.env.create_test_iterator(data_type, params=params, data_path=self.trainer.data_path)
        ...
        scores[f'{data_type}_xe_loss'] = xe_loss / _n_total
        scores[f'{data_type}_acc'] = 100. * _n_valid / _n_total
        ...
        for i in range(len(n_total)):
            ...
            scores[f'{data_type}_acc_{i}'] = 100. * n_valid[i].item() / max(n_total[i].item(), 1)

    def enc_dec_step_beam(self, data_type, scores):
        """
        ...
        # iterator
        iterator = env.create_test_iterator(data_type, params=params, data_path=self.trainer.data_path)
        eval_size = len(iterator.dataset)
        ...
        # compute perplexity and prediction accuracy
        assert _n_total == eval_size or self.trainer.data_path
        scores[f'{data_type}_beam_acc'] = 100. * _n_valid / _n_total
        ...
        for i in range(len(n_total)):
            ...
            scores[f'{data_type}_beam_acc_{i}'] = 100. * n_valid[i].sum().item() / max(n_total[i].item(), 1)
