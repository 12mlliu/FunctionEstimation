...
class Trainer(object):
    ...
    def __init__(self, modules, env, params):
        ...
        if params.reload_data != '':
            ...
            self.data_path = [x for x in params.reload_data.split(',') if len(x) > 0]
            print(self.data_path)
            assert all(os.path.isfile(path) for path in self.data_path) 
            ...
            #self.data_path = [train_path, valid_path, test_path for train_path, valid_path, test_path in s]
        ...
        if not params.eval_only:
            ...
            self.dataloader = iter(self.env.create_train_iterator(params, self.data_path))
    ...
    def export_data(self):
        ...
        env = self.env
        #d, f
        (d, d_len), (f, f_len), _ = self.get_batch()
        for i in range(len(f_len)):
            ...
            infix_str = infix.replace('f(x)', 'y').replace('Derivative(y,x)', "y'").replace("Derivative(y',x)", "y''")
            #infix2_str = infix2.replace('f(x)', 'y').replace('Derivative(y,x)', "y'").replace("Derivative(y',x)", "y''")
            infix_str = re.sub(r'(?<![a-zA-Z0-9])\(([a-zA-Z0-9]+)\)(?<![a-zA-Z0-9])', r'\1', infix_str)
            #infix2_str = re.sub(r'(?<![a-zA-Z0-9])\(([a-zA-Z0-9]+)\)(?<![a-zA-Z0-9])', r'\1', infix2_str)
            self.file_handler_prefix.write(f'{prefix_str}\t{data_str}\n')
        ...
        self.stats['processed_e'] += d_len.size(0)
        self.stats['processed_w'] += (f_len - 2).sum().item()
